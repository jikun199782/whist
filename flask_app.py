...
# (PythonAnywhere may have an older playcard.py without it)
...
# =========================================================================
#  Existing single-player / AI routes  (unchanged)
# =========================================================================
...
def _resolve_trick(game_id):
    ...
    trick = gs['tricks'][-1]
    winner = determine_trick_winner(trick, gs['trump_suit'])
    ...
    if scores_sn >= WINNING_TRICKS:
        stop_type = 'game_over'
        msg = "Congratulations! South-North wins the game!"
        msg_class = 'win-message'
        new_leader = None
    elif scores_ew >= WINNING_TRICKS:
        stop_type = 'game_over'
        msg = "East-West wins the game. Better luck next time!"
        msg_class = 'lose-message'
        new_leader = None
    elif (scores_sn + scores_ew) >= 13:
        stop_type = 'game_over'
        if scores_sn > scores_ew:
            msg = "Game over! South-North wins!"
            msg_class = 'win-message'
        else:
            msg = "Game over! East-West wins!"
            msg_class = 'lose-message'
        new_leader = None
    else:
        stop_type = 'new_trick'
...
def _auto_play_ai(game_id):
    """After a human plays, auto-play all consecutive AI seats until
    either the trick completes or the next human's turn arrives.
    Returns the updated game_state dict.
    """
    while True:
        ...
        if seat and seat['player_type'] == 'human' and seat['is_connected']:
            ...
            if game_store.get_current_trick(game_id).get(
                    game_store.get_game(game_id)['leader']) is None \
                    and next_pos == game_store.get_game(game_id)['leader']:
        ...
        trump_suit = game_store.get_game(game_id)['trump_suit']
        card = ai_choose_card(hand, trick, trump_suit, next_pos)
...
def multi_status(game_id):
    ...
    if gs['stop_type'] in ('lead_card', 'follow_card'):
        ...
        # Only the leader of the new trick can click "New Trick"
        is_my_turn = (my_pos == leader)
...
def multi_new_trick(game_id):
    """Start the next trick (called by 'New Trick' button in multiplayer mode)."""
    gs = game_store.get_game_state(game_id)
    if not gs:
        return redirect(url_for('select'))

    my_pos, my_token = _get_player_id()
    if gs['stop_type'] not in ('new_trick', 'waiting'):
        return _redirect_game(game_id, my_pos, my_token)
