...
def init_db() -> None:
    ...
    """)
    conn.commit()
    # Backward compat: add difficulty column if missing
    try:
        conn.execute("ALTER TABLE games ADD COLUMN difficulty TEXT DEFAULT 'easy'")
        conn.commit()
...
def create_game(game_id: str, hands: dict, trump_suit: str,
                trump_suit_name: str, difficulty: str = 'easy') -> None:
    ...
    Args:
        ...
        difficulty: 'easy' or 'hard' AI difficulty.
    """
    conn = _get_conn()
    conn.execute("""
        INSERT INTO games (id, trump_suit, trump_suit_name, difficulty)
        VALUES (?, ?, ?, ?)
    """, (game_id, trump_suit, trump_suit_name, difficulty))
...
def set_difficulty(game_id: str, difficulty: str) -> None:
    """Update the AI difficulty for a game (host only, waiting phase)."""
    conn = _get_conn()
    conn.execute("UPDATE games SET difficulty = ? WHERE id = ?",
                 (difficulty, game_id))
    conn.commit()
    conn.close()
    bump_version(game_id)
